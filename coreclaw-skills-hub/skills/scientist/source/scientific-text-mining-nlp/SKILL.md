...
description: |
 NLP skill。 NER（gene/disease/drug/compound）
 extraction（PPI / DDI / GDA）literaturegraphconstruction
 citationnetworkanalysispipeline。
 PubTator / SemanticScholar / EuropePMC dataintegration。
---
...
def mark_entities(text, ent_a, ent_b):
 """generation。"""
 # implementation: @ENTITY_A@ / @ENTITY_B@ 
...
|---|---|---|
| PubTator | `PubTator3_LiteratureSearch` | NER literaturesearch |
| PubTator | `PubTator3_EntityAutocomplete` | |
| PubMed | `PubMed_search_articles` | PubMed literaturesearch |
| PubMed | `PubMed_get_cited_by` | cited-by article retrieval |
| PubMed | `PubMed_get_related` | paperretrieval |
| SemanticScholar | `SemanticScholar_search_papers` | papersearch |
| EuropePMC | `EuropePMC_search_articles` | EuropePMC search |
| EuropePMC | `EuropePMC_get_fulltext` | allretrieval |
| EuropePMC | `EuropePMC_get_references` | citationliteratureretrieval |
| OpenAlex | `openalex_search_works` | OpenAlex search |
| DBLP | `DBLP_search_publications` | CS literaturesearch |
