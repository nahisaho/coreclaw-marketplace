...
description: |
 paper's changeminskill。's diff generation、
 change's（deletion/addition）、summaryautomatedgeneration、
 peer reviewand 's is performed。
 「 」「change 」「diff 」 。
tu_tools:
 - key: crossref
 name: Crossref
 description: 's citationverification
---
...
|--------|---------|--------|
| `crossref` | Crossref | 's citationverification |
